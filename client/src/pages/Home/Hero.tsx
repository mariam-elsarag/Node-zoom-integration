...
const Hero = () => {
  const { data } = useGetData("/api/meet/upcoming");

  return (
    <div className="hero w-full object-cover h-[200px] md:h-[250px] lg:h-[280px] rounded-2xl">
      <header
        className={`py-5 sm:py-9 pl-5 sm:pl-10 h-full flex flex-col ${
          data?.have_upcoming_meet ? "justify-between" : "justify-end"
        }`}
      >
        {data?.have_upcoming_meet && (
          <div className="bg-white/5 w-fit py-2.5 px-3.5 rounded-sm">
            <span>Upcoming Meeting at:{data?.meet_time}</span>
          </div>
        )}
